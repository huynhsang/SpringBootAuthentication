...
public class DataTesting implements ApplicationListener<ContextRefreshedEvent>{
	...
	public void onApplicationEvent(ContextRefreshedEvent arg0) {
	    ...
	    if (userDAO.findByUsername("admin") == null) {
	    	Date date = new Date();
	        User admin = new User("admin", passwordEncoder.encode("admin"), date);
	    ...
	    if (userDAO.findByUsername("user") == null) {
	    	Date date = new Date();
	        User user = new User("user", passwordEncoder.encode("user"), date);
	        List<Authority> authorities = new ArrayList<Authority>();
	    ...
	    if (userDAO.findByUsername("staff") == null) {
	    	Date date = new Date();
	        User staff = new User("staff", passwordEncoder.encode("staff"), date);
	        List<Authority> authorities = new ArrayList<Authority>();
